...
class RestServiceAdaptor (icsApi: BSIRestIcsApi)
{
  ...
  def accountByEmail(email:String):List[Account]=
  {
  ...
  def deleteById(id:String):Try[Option[Account]]=
  {
    val icsSession = icsApi.createSession()
  ...
  def createNewAccount(account: Account): Try[Account] =
  {
...
//check for existing active account with same email
  ...
  def validateAccount(account : Account):Try[Account] =
  {
    Try({
      account.email match {
  ...
  def updateAccount(account: Account, accountId: String): Try[Account] = {
    ...
    def accountDTOtoIdentity(accountImmutable: Account): Identity = {
    ...
    def convertToAccountDTO(id: Identity): Account = {
